...
public class DefaultDAO<K, V> implements DAO<K, V> {
	...
	public List<V> query(String field, Object value) {
		log.debug("query: field={}, value={}", field, value);
		String q = String.format("select distinct * from %s where %s=$1", region.getFullPath(), field);
		Object[] args = new Object[] { value };
		return query(q, args);
	}

	protected List<V> query(String query, Object[] args) {
		if (log.isDebugEnabled()) {
			log.debug("query: query={}, args={}", query, Arrays.toString(args));
		}

		try {
			Query q = service.newQuery(query);
			@SuppressWarnings("unchecked")
			SelectResults<V> res = (SelectResults<V>) (args == null || args.length == 0 ? q.execute()
					: q.execute(args));
