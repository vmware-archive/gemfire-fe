...
public class MongoDBQueryCacheLoader<K, V> implements CacheLoader<K, V>, Declarable {
    ...
    public V load(LoaderHelper<K, V> helper) throws CacheLoaderException {
        String query = (String)helper.getArgument();
        
        if (query == null) {
            throw new CacheLoaderException("Missing query");
        }
